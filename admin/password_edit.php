...
if (isset($_POST['edit'])) {
    ...
    if(empty($curr_password) || empty($password_new1)|| empty($password_new2)){
        ...
        echo "<script>window.open('index.php?error3','_self')</script>";

  }elseif ($password_new1!=$password_new2){
        echo "<script>alert('New password and Conform password doesnot match')</script>";
        echo "<script>window.open('index.php?error1','_self')</script>";
       }      
        ...
        $sql = "UPDATE `users` SET `password`='$password_new1' WHERE `email` = '$user'";
          $result = $connect->query($sql);
            if($result){
                ...
                echo "<script>window.open('../logout.php?pass_changed','_self')</script>";
                exit();
            } else{
                echo "<script>alert('Oops! Something went wrong. Please try again later')</script>";
                echo "<script>window.open('index.php?error2','_self')</script>";
            }                  
  }else{
