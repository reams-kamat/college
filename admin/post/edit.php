	...
	<div class="container-fluid">
		...
		<div class="row sec_row">
			<div  class="col-md-2 side_bar"  >
				...
				$pid =$_GET['edit'];
				// var_dump($pid); die();
				$sql_query="SELECT * FROM `post` WHERE `id` = '$pid'";
				$result = $connect->query($sql_query);
				if($result){
				while ($row = $result->fetch_assoc()) {
					$iid=$row['id'];
			...
			<div  class="col-md-10 content_bar">
				<div style="margin:50px;">
					<a class="btn btn_color" href="delete.php?id=<?php echo $pid ; ?>"  type="submit">Delete This Post</a>&nbsp;&nbsp;&nbsp;
				...
				<div style="margin:50px;" >
					<form  method="POST" enctype="multipart/form-data">
						...
						<div class="form-group">
							...
							<input type="file" name="image" style="height: 100px;padding-top: 70px;" class="form-control" id="exampleInputimage1" placeholder="image" value="<?php echo $row['image']; ?>">
							<img src="images/<?php echo $row['image']; ?>" width="90" height="90" style="margin-right: 5px;margin-top: -95px;" align="right">
						</div>
						<div class="form-group">
							<label for="exampleInputEmail1">Content</label>
							<textarea name="content1" required="true" id="content1"><?php echo $row['content']; ?></textarea>
			...
			if ($image_type=="image/jpeg" or $image_type=="" or $image_type=="image/jpg" or $image_type=="image/png" or $image_type=="image/gif") {
				if ($image_size<=2000000) {
					move_uploaded_file($image_tmp,"images/$image_name");
							// <!-- // echo "$iid;"; -->
						$sql_query ="UPDATE `post` SET `title`='$title',`author`='$author',`image`='$image_name',`content`='$content1',`date`='$date',`status`='approved' WHERE `id` = '$iid'"; 
