 ...
 if(!empty($image_name)){
    ...
    if ($password==$password1) {
        if(empty($name) || empty($phn_new)|| empty($address) || empty($email) || empty($password1) ){
            ...
            echo "<script>window.open('index.php?error','_self')</script>";
         }

         elseif (!preg_match("/^[a-zA-Z ]*$/",$name)){
            echo "<script>alert('Invalid Name')</script>";
            echo "<script>window.open('index.php?invalid_name','_self')</script>";
           
          }
        elseif(!filter_var($email, FILTER_VALIDATE_EMAIL)) {  
            echo "<script>alert('Invalid Email')</script>";
            echo "<script>window.open('index.php?invalid_email','_self')</script>";      
          }      
       else
       {
            ...
            if ($check_user==0) {
                    if ($image_type=="image/jpeg" or $image_type=="image/jpg"or $image_type=="" or $image_type=="image/png" or $image_type=="image/gif") {
                              if ($image_size<=2000000) {
                                ...
                                echo "<script>window.open('index.php?large_size','_self')</script>";
                              }

                      }
                  else{
                 echo "<script>alert('Invalid file format')</script>";
                 echo "<script>window.open('index.php?invalid_file_format','_self')</script>"; 
                  }
            }elseif ($check_user==1) {
                if ($email==$user && $username==$_SESSION['username']) {
                 ...
                 echo "<script>window.open('index.php?invalid_file_format','_self')</script>"; 
                    ...
                    echo "<script>window.open('index.php?failed','_self')</script>";  
                }
         ...
         echo "<script>window.open('index.php?failed2','_self')</script>"; 
    }
}
?>